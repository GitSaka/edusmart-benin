    ...
    Credentials({
      ...
      async authorize(credentials) {
        try {
          ...
          if (!typed.identifier || !typed.password) return null

          const identifier = typed.identifier.trim()
          const password = typed.password
          const role = typed.role
          const ecoleCode = typed.ecoleCode?.trim()
